 ...
 */
public class NeighbourMeterPanelController implements ActionListener, KeyListener, MouseListener {
    
    private Neighbour neighbour;
    
    private NeighbourMeterPanelView view;
    private NeighbourMeterPanelModel model;
    
    public NeighbourMeterPanelController(NeighbourMeterPanelModel model, NeighbourMeterPanelView view) {
        this.view = view;
        this.model = model;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (action.equals(ActionKey.NEIGHBOUR_METER_CREATE)) {
            createNeighbourMeter();
        }
        else if (action.equals(ActionKey.NEIGHBOUR_METER_SAVE)) {
            saveNeighbour();
        }   
        else if (action.equals(ActionKey.NEIGHBOUR_METER_DELETE)) {
            deleteMeter();
        }        
        else if (action.equals(ActionKey.NEIGHBOUR_METER_SAVE)) {
            saveMeter();
    ...
    public void createNeighbourMeter() {
        //Comment comment = new Comment();  
        NeighbourMeter neighbourMeter = model.createNeighbourMeter();
        if (neighbourMeter!=null) {
            //view.addComment(comment);
            //model.showMessageNeighbourMeterCreated();
        }        
    }
    ...
    public void setNeighbour(Neighbour neighbour) {
        ...
        List<MeterNodeView> meterViewList = new ArrayList();
        for (NeighbourMeterView nmv : nmvList) {
            MeterNodeView meterView = new MeterNodeView();
            meterView.buttonActionListeners(this);
            meterView.writeData(nmv);
            meterViewList.add(meterView);
        }
        view.writeData(meterViewList);
    }
